...
| Food prices (consumer) | real-time capable | BI Harga Pangan scraper/API (`price_type_id=1`, Pasar Tradisional) and Bapanas Panel Harga when endpoint access is stable | Bapanas Panel Harga states daily updates by enumerators; main panel may be under maintenance, so API availability must be validated before production. |
| Food prices (producer) | real-time capable | BI Harga Pangan scraper/API (`price_type_id=4`, Produsen) | Stored in `commodity_prices` with `price_level='producer'`, separate from consumer-level rows. Powers the producer-retail margin view (`v_producer_retail_margin`) and `/api/prices/producer-margin` endpoint - a distribution-inefficiency signal, not just a de-mocking fix. |
